...
#load all admix output
#qfilelist <- list.files(path=admixpath, pattern = '*.Q', full.names = TRUE)
#pfilelist <- list.files(path=admixpath, pattern = '*.P', full.names = TRUE)
#lfilelist <- list.files(path=admixpath, pattern = '*.out', full.names = TRUE)
#fhand <- 'CM023248.thin0.5_50.10.0.1'
#load all admix output into a list of objects
#admixoutput = list()
#for (a in seq(1,length(qfilelist))) {
#  my_admix <- loadAdmixture(qfilelist[a], pfilelist[a], lfilelist[a])
#  admixoutput[[a]] <- my_admix
#}

#plot cv diagnostics
#starmie::bestK(starmie::admixList(admixoutput))
#ggsave('~/Projects/AsGARD/figures/bestk.svg')

#make admix object
#admixlistall <- starmie::admixList(admixoutput)

# plot multik from starmie
#admix_df_all <- starmie::plotMultiK(admixlistall, populations=df_samples[c('callset_order','pop_code')], plot = TRUE)+theme_classic()+theme(axis.text.x = element_blank(), axis.ticks.x=element_blank(), axis.title.x = element_blank())
#ggsave(filename="~/Projects/AsGARD/figures/admixall.svg",admix_df_all, width=14, height=8)

# Now plot haplonet output
