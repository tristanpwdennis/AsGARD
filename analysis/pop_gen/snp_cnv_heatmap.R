...
#to remove
samples_to_remove <- c('AfNH6','AfND5','C1aSud006','C1aSud056','C1aSud064','C1aSud139','C1aSud147','DAsgPak010','DAsgCPak034','Dubt_I_11',"Jiga_I_06","PMJDE3","PMJDG3","R10","R17C","R3C" )

#modify sample id properly
cnv$sample_id_modified <- gsub(".merged","",gsub("_EKD.*","",cnv$sample_id))

#remove guff samples
cnv <- cnv[!cnv$sample_id_modified %in% samples_to_remove]
write_csv(x = data.frame(cnv), file='/Users/dennistpw/Projects/AsGARD/data/cnv/cnv_all.csv')
...
#and for metabolics
metabolic_snp_data <- fread('~/Projects/AsGARD/data/metabolic_snps.csv')
metabolic_snp_data <- left_join(metabolic_snp_data, cnv_genes, by=c('AnsteID'))
write_csv()
